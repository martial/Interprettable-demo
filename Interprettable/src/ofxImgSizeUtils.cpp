...
ofRectangle  ofxImgSizeUtils::getCenteredRect( int maxw, int maxh, float currw, float currh, bool keepInto ) {
    
    ofRectangle rect;
    ofPoint size = getSizeRatio(maxw, maxh, currw, currh, keepInto);
    rect.set(maxw *.5 - size.x*.5, maxh *.5 - size.y*.5, size.x, size.y);
